...
export const Sidebar: React.FC<SidebarProps> = ({
    ...
    return (
        <>
            ...
            {isMobileOpen && (
                <div
                    className="fixed inset-0 bg-green-950/80 z-40 lg:hidden backdrop-blur-sm transition-opacity"
                    onClick={() => setIsMobileOpen(false)}
                />
            )}

            <aside
                className={`fixed top-0 left-0 z-50 h-screen bg-[#06160e] border-r border-green-900/50 text-white transition-all duration-300 shadow-2xl flex flex-col font-sans
                ${isCollapsed ? 'w-20' : 'w-72'} 
                ${isMobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
            >
                {/* Bagian Merek / Logo Perusahaan */}
                <div className={`h-20 flex items-center border-b border-green-900/80 bg-[#030d08] transition-all duration-300 ${isCollapsed ? 'justify-center px-0' : 'px-6'}`}>
                    <div className="flex items-center gap-4 overflow-hidden w-full justify-center lg:justify-start">
                        <div className={`bg-green-950 border-2 border-green-800 flex items-center justify-center ml-5 flex-shrink-0 shadow-lg shadow-emerald-900/20 transition-all duration-300 ${isCollapsed ? 'w-10 h-10 rounded-2xl rounded-tr-none' : 'w-12 h-12 rounded-2xl rounded-tr-none'}`}>
                            <Layers size={isCollapsed ? 20 : 24} className="text-white" />
                        </div>
                        ...
                        {!isCollapsed && (
                            <div className="flex flex-col animate-in fade-in duration-300 truncate">
                                ...
                                <span className="text-[9px] font-black text-white uppercase tracking-widest block mt-1.5 opacity-90 truncate">
                                        ...
                                        {hasChildren ? (
                                            <div className="space-y-1">
                                                <button
                                                    ...
                                                    ${isParentActive ? 'bg-green-950 text-white font-black border border-green-900/50 shadow-inner' : 'hover:bg-green-900/40 text-white'}
                                                    ${isCollapsed ? 'w-12 h-12 justify-center p-0 mx-auto' : 'px-4 py-3.5'}`}
                                                >
                                                    <div className="flex items-center gap-3">
                                                        <span className={`flex items-center justify-center transition-colors text-white group-hover:text-white`}>
                                                            {renderIcon(menu.icon)}
                                                        </span>
                                                        {!isCollapsed && <span className="text-xs font-bold tracking-wide uppercase">{menu.title}</span>}
                                                    </div>
                                                    {!isCollapsed && (
                                                        <ChevronDown size={14} className={`transition-transform duration-200 text-white group-hover:text-white ${isOpen ? 'rotate-180' : ''}`} />
                                                    )}
                                                </button>

                                                {/* Dropdown Laci Anak */}
                                                <div className={`overflow-hidden transition-all duration-300 ${isOpen && !isCollapsed ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                                                    <ul className="space-y-1 pl-10 pr-2 py-2 relative before:absolute before:left-6 before:top-2 before:bottom-2 before:w-px before:bg-green-900/50">
                                                            ...
                                                            return (
                                                                <li key={child.id}>
                                                                    <Link
                                                                        ...
                                                                        ${isChildActive
                                                                                ? 'bg-green-900/50 text-white font-black border border-green-800/50'
                                            ...
                                            <Link
                                                ...
                                                ${location.pathname === menu.path
                                                        ? 'bg-green-950 text-white font-black shadow-lg shadow-green-900/40 tracking-wider uppercase border border-green-900/50'
                                                        : 'hover:bg-green-900/40 text-white font-bold uppercase tracking-wider'}
                                                ${isCollapsed ? 'w-12 h-12 justify-center p-0 mx-auto' : 'px-4 py-3.5'}`}
                                            >
                                                <span className={`flex items-center justify-center transition-colors text-white group-hover:text-white`}>
                                                    {renderIcon(menu.icon)}
