...
export const NavbarBuilderPage: React.FC = () => {
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto pb-10 font-sans animate-in fade-in duration-300">
      ...
      {/* Header Panel */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 bg-white p-8 rounded-[2rem] rounded-tr-none border-2 border-green-50 shadow-sm hover:border-emerald-100 transition-colors">
        <div>
        ...
        >
          {isSaving ? <Loader2 className="animate-spin text-emerald-400" size={16} /> : <Save size={16} className="text-emerald-400 group-hover/btn:rotate-12 transition-transform" />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white p-6 rounded-[2rem] rounded-tr-none border-2 border-green-50 shadow-sm space-y-6 hover:border-emerald-100 transition-colors">
            <div className="border-b-2 border-green-50 pb-4">
              <h2 className="text-sm font-black text-green-950 uppercase tracking-widest flex items-center gap-2">
                <Plus size={16} className="text-emerald-500" /> Injeksi Tautan Baru
              </h2>
            </div>

            {/* Opsi Injeksi Cerdas: Halaman Statis */}
            <div className="space-y-2 bg-green-50/50 p-4 rounded-2xl border border-green-100">
              <label className="text-[11px] font-black text-slate-500 uppercase flex items-center gap-2">
                <FileText size={14} className="text-emerald-600" /> Hubungkan Halaman Statis
              </label>
              <select
                onChange={e => {
                ...
                className="w-full bg-white border border-green-100 p-3 rounded-xl text-xs font-bold text-slate-800 outline-none focus:border-emerald-500 shadow-sm"
              >
            ...
            <div className="space-y-2 bg-green-50/50 p-4 rounded-2xl border border-green-100">
              <label className="text-[11px] font-black text-slate-500 uppercase flex items-center gap-2">
                <FolderPlus size={14} className="text-emerald-600" /> Hubungkan Arsip Kategori
              </label>
              <select
                onChange={e => {
            ...
            <div className="flex items-center gap-3 my-2">
              <div className="h-0.5 bg-green-50 flex-1" />
              <span className="text-[9px] font-black text-emerald-600 uppercase tracking-widest">Atau Kustomisasi</span>
              <div className="h-0.5 bg-green-50 flex-1" />
            </div>
            ...
            <form onSubmit={handleAddMenu} className="space-y-5">
              <div>
                ...
                <select
                  ...
                  className="w-full border border-green-100 p-3.5 rounded-2xl rounded-tr-none text-xs font-bold text-green-900 bg-emerald-50 outline-none focus:border-emerald-500 shadow-sm"
                >
              ...
              <div>
                <label className="block text-[11px] font-black text-slate-500 mb-2 uppercase tracking-widest flex justify-between">
                  ...
                  <span className="text-[9px] text-emerald-500 italic font-medium">kosongkan jika dropdown</span>
                </label>
                <div className="relative">
                  <Link2 className="absolute left-4 top-1/2 -translate-y-1/2 text-emerald-600" size={16} />
                  <input 
                    value={urlInput} onChange={e => setUrlInput(e.target.value)}
                    className="w-full pl-10 pr-4 py-3.5 bg-white border border-green-100 rounded-2xl rounded-tr-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none text-sm font-mono text-slate-800 shadow-sm transition-all" 
                    placeholder="e.g. /p/rute atau #"
                  />
                </div>
              </div>

              <button type="submit" className="w-full py-3.5 bg-green-950 hover:bg-green-900 text-white font-black rounded-2xl rounded-tr-none text-xs uppercase tracking-widest shadow-lg transition-all active:scale-95">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-6 lg:p-8 rounded-[2rem] rounded-tr-none border-2 border-green-50 shadow-sm space-y-6 hover:border-emerald-100 transition-colors">
            <div className="flex items-center justify-between border-b-2 border-green-50 pb-4">
              <h2 className="text-sm font-black text-green-950 uppercase tracking-widest flex items-center gap-2">
                <Layers size={16} className="text-emerald-500" /> Pohon Navigasi Aktif
              </h2>
              <span className="text-[10px] font-black bg-emerald-50 text-emerald-600 px-3 py-1.5 rounded-xl uppercase tracking-widest border border-emerald-100">{navItems.length} Induk</span>
            </div>

            {isLoading ? (
              <div className="py-16 text-center"><Loader2 className="animate-spin mx-auto text-emerald-500" size={32} /></div>
            ) : navItems.length === 0 ? (
              <div className="py-16 text-center border-2 border-dashed border-green-100 rounded-[2rem] bg-slate-50">
              ...
              <div className="space-y-5">
                {navItems.map((item, index) => (
                  <div key={item.id} className="border-2 border-green-50 rounded-2xl bg-white shadow-sm overflow-hidden transition-all group">
                    
                    {/* Bar Induk Utama */}
                    <div className="bg-green-50/50 p-4 flex items-center justify-between gap-3 border-b border-green-100">
                      <div className="flex items-center gap-3 flex-1">
                        <span className="w-8 h-8 bg-green-950 text-white rounded-xl flex items-center justify-center font-black text-xs flex-shrink-0 shadow-sm">
                          {index + 1}
                        </span>
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 flex-1">
                          <input 
                            value={item.label} onChange={e => handleUpdateItem(item.id, 'label', e.target.value)}
                            className="font-bold text-sm text-slate-900 bg-white border border-green-100 px-3 py-2 rounded-xl w-full sm:w-1/2 outline-none focus:border-emerald-500 shadow-sm"
                            placeholder="Label Induk"
                          />
                          <input 
                            value={item.url} onChange={e => handleUpdateItem(item.id, 'url', e.target.value)}
                            className="font-mono text-sm text-emerald-600 bg-white border border-green-100 px-3 py-2 rounded-xl w-full sm:w-1/2 outline-none focus:border-emerald-500 shadow-sm"
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0">
                        <button type="button" onClick={() => moveMenu(index, 'UP')} disabled={index === 0} className="p-1.5 text-slate-400 hover:text-green-950 disabled:opacity-20 transition-colors bg-white rounded-lg border border-slate-200 shadow-sm"><ChevronUp size={16}/></button>
                        <button type="button" onClick={() => moveMenu(index, 'DOWN')} disabled={index === navItems.length - 1} className="p-1.5 text-slate-400 hover:text-green-950 disabled:opacity-20 transition-colors bg-white rounded-lg border border-slate-200 shadow-sm"><ChevronDown size={16}/></button>
                        <div className="w-px h-6 bg-green-100 mx-2" />
                    ...
                    <div className="p-4 bg-white space-y-3 pl-10 border-l-[3px] border-emerald-400 ml-4 my-2">
                      ...
                      {item.subMenus.length === 0 ? (
                        ...
                        <div className="space-y-3">
                          {item.subMenus.map((sub, sIdx) => (
                            <div key={sIdx} className="flex items-center gap-3 bg-green-50/30 p-3 rounded-xl border border-green-50">
                              <span className="text-[11px] font-black text-emerald-600 w-5">{sIdx + 1}.</span>
                              <input 
                                value={sub.label} onChange={e => handleUpdateSubItem(item.id, sIdx, 'label', e.target.value)}
                                className="text-xs font-bold text-slate-800 bg-white border border-green-100 px-3 py-2 rounded-lg w-1/3 outline-none focus:border-emerald-500 shadow-sm"
                                placeholder="Sub Label"
                              />
                              <input 
                                value={sub.url} onChange={e => handleUpdateSubItem(item.id, sIdx, 'url', e.target.value)}
                                className="text-xs font-mono text-emerald-600 bg-white border border-green-100 px-3 py-2 rounded-lg flex-1 outline-none focus:border-emerald-500 shadow-sm"
                                placeholder="/p/rute"
                              />
                              
                              <div className="flex items-center gap-1">
                                <button type="button" onClick={() => moveSubMenu(item.id, sIdx, 'UP')} disabled={sIdx === 0} className="p-1 text-slate-400 hover:text-green-900 disabled:opacity-20"><ChevronUp size={14}/></button>
                                <button type="button" onClick={() => moveSubMenu(item.id, sIdx, 'DOWN')} disabled={sIdx === item.subMenus.length - 1} className="p-1 text-slate-400 hover:text-green-900 disabled:opacity-20"><ChevronDown size={14}/></button>
