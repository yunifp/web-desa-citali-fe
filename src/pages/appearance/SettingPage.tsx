...
export const SettingPage: React.FC = () => {
  ...
  if (isLoading) return <div className="py-24 text-center"><Loader2 className="animate-spin mx-auto text-emerald-500" size={40} /></div>;

  return (
    <div className="space-y-8 max-w-5xl mx-auto pb-10 font-sans animate-in fade-in duration-300">
      
      <div className="bg-white p-6 lg:p-8 rounded-[2rem] rounded-tr-none border-2 border-green-50 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 hover:border-emerald-100 transition-colors">
        <div>
        ...
        >
          {isSaving ? <Loader2 className="animate-spin text-emerald-400" size={16} /> : <Save size={16} className="text-emerald-400 group-hover/btn:rotate-12 transition-transform" />} Simpan Parameter
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white p-6 lg:p-8 rounded-[2rem] rounded-tr-none border-2 border-green-50 shadow-sm space-y-6 hover:border-emerald-100 transition-colors">
          <div className="border-b-2 border-green-50 pb-4">
            <h2 className="text-sm font-black text-green-950 uppercase tracking-widest flex items-center gap-2">
              <Megaphone size={16} className="text-emerald-500" /> Banner Pengumuman Navigasi
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select 
                ...
                className="w-full p-3.5 bg-white border border-green-100 rounded-2xl rounded-tr-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none text-sm font-bold text-slate-800 shadow-sm transition-all"
              >
            ...
            <div>
              ...
              <input 
                ...
                className="w-full p-3.5 bg-white border border-green-100 rounded-2xl rounded-tr-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none text-sm font-mono text-slate-800 shadow-sm transition-all"
            ...
            <div className="md:col-span-2">
              ...
              <input 
                ...
                className="w-full p-3.5 bg-white border border-green-100 rounded-2xl rounded-tr-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none text-sm font-bold text-slate-900 shadow-sm transition-all"
        ...
        <div className="bg-white p-6 lg:p-8 rounded-[2rem] rounded-tr-none border-2 border-green-50 shadow-sm space-y-6 hover:border-emerald-100 transition-colors">
          <div className="border-b-2 border-green-50 pb-4 flex justify-between items-center">
            <h2 className="text-sm font-black text-green-950 uppercase tracking-widest flex items-center gap-2">
              <Layers size={16} className="text-emerald-500" /> Pengaturan Bar Afiliasi (Top Bar)
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="space-y-4">
              <div className="flex justify-between items-center mb-2">
                ...
                <button type="button" onClick={handleAddTopbarLink} className="text-[10px] px-3 py-1.5 bg-emerald-50 text-emerald-600 border border-emerald-100 font-black rounded-lg hover:bg-emerald-100 transition-colors flex items-center gap-1 uppercase tracking-widest">
                  <Plus size={12}/> Tambah
                </button>
              </div>
              
              {topbarLinks.length === 0 ? (
                <p className="text-xs text-slate-400 italic font-medium p-4 bg-slate-50 border-2 border-dashed border-green-100 rounded-2xl rounded-tr-none text-center">Tidak ada tautan afiliasi. Gunakan tombol Tambah.</p>
              ) : (
                <div className="space-y-3">
                  {topbarLinks.map((link, idx) => (
                    <div key={idx} className="flex gap-2 items-center bg-green-50/50 p-3 rounded-2xl border border-green-100">
                      <input 
                        value={link.label} onChange={e => handleUpdateTopbarLink(idx, 'label', e.target.value)}
                        className="text-xs font-bold text-slate-900 bg-white border border-green-100 p-2.5 rounded-xl w-1/2 outline-none focus:border-emerald-500 shadow-sm"
                        placeholder="Label Afiliasi" 
                      />
                      <input 
                        value={link.url} onChange={e => handleUpdateTopbarLink(idx, 'url', e.target.value)}
                        className="text-xs font-mono text-emerald-600 bg-white border border-green-100 p-2.5 rounded-xl flex-1 outline-none focus:border-emerald-500 shadow-sm"
                        placeholder="URL / #" 
                      />
            ...
            <div className="space-y-4 border-t-2 lg:border-t-0 lg:border-l-2 border-green-50 lg:pl-8 pt-6 lg:pt-0">
        ...
        <div className="bg-white p-6 lg:p-8 rounded-[2rem] rounded-tr-none border-2 border-green-50 shadow-sm space-y-6 hover:border-emerald-100 transition-colors">
          <div className="border-b-2 border-green-50 pb-4">
            <h2 className="text-sm font-black text-green-950 uppercase tracking-widest flex items-center gap-2">
              <Globe size={16} className="text-emerald-500" /> Atribut Merek Pokok
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3 p-5 bg-green-50/50 rounded-2xl border border-green-100 shadow-sm">
              <span className="text-xs font-black text-green-950 block uppercase tracking-widest">Aset Visual Header (Navbar)</span>
              <div className="flex items-center gap-5">
                <div className="w-32 h-16 bg-white rounded-xl border border-green-200 flex items-center justify-center overflow-hidden relative p-1 shadow-sm">
                  {settings.site_logo ? (
                  ...
                  {isUploadingLogo && <div className="absolute inset-0 bg-green-950/60 flex items-center justify-center"><Loader2 className="animate-spin text-white" size={16}/></div>}
                </div>
                <div className="flex-1 space-y-2">
                  ...
                  <label htmlFor="logoUp" className="px-4 py-2.5 bg-white border border-green-200 hover:bg-green-50 hover:border-emerald-400 text-xs font-black rounded-xl cursor-pointer block text-center shadow-sm text-green-950 transition-colors uppercase tracking-widest">Unggah Logo</label>
            ...
            <div className="space-y-3 p-5 bg-slate-900 rounded-2xl border border-slate-800 shadow-sm">
              ...
              <div className="flex items-center gap-5">
                <div className="w-32 h-16 bg-slate-950 rounded-xl border border-slate-800 flex items-center justify-center overflow-hidden relative p-1 shadow-sm">
                  ...
                  {isUploadingFooterLogo && <div className="absolute inset-0 bg-slate-950/60 flex items-center justify-center"><Loader2 className="animate-spin text-emerald-400" size={16}/></div>}
                </div>
                <div className="flex-1 space-y-2">
            ...
            <div className="md:col-span-2">
              ...
              <textarea 
                ...
                className="w-full p-4 bg-white border border-green-100 rounded-2xl rounded-tr-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none text-sm font-medium text-slate-700 shadow-sm transition-all resize-none leading-relaxed h-20" 
        ...
        <div className="bg-white p-6 lg:p-8 rounded-[2rem] rounded-tr-none border-2 border-green-50 shadow-sm space-y-6 hover:border-emerald-100 transition-colors">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b-2 border-green-50 pb-4">
            <h2 className="text-sm font-black text-green-950 uppercase tracking-widest flex items-center gap-2">
              <Layers size={16} className="text-emerald-500" /> Penataan Kolom Navigasi Bawah
            </h2>
            <button type="button" onClick={handleAddColumn} className="px-4 py-2 bg-green-950 hover:bg-green-900 text-white font-black rounded-xl text-xs flex items-center gap-2 transition-colors uppercase tracking-widest shadow-sm">
              <Plus size={14} className="text-emerald-400"/> Tambah Kolom
            </button>
          </div>

          <div className="space-y-5">
            {footerCols.length === 0 ? (
              <div className="border-2 border-dashed border-green-100 rounded-[2rem] p-12 text-center bg-slate-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {footerCols.map((col, cIdx) => (
                  <div key={cIdx} className="border-2 border-green-50 rounded-2xl rounded-tr-none bg-green-50/30 p-5 space-y-4 relative group">
                    <div className="flex items-center gap-3 pr-8">
                      <input 
                        value={col.title} onChange={e => handleUpdateColTitle(cIdx, e.target.value)}
                        className="font-black text-sm text-slate-900 bg-white border border-green-100 p-3 rounded-xl flex-1 outline-none focus:border-emerald-500 shadow-sm"
                    ...
                    <div className="space-y-3 pt-4 border-t-2 border-green-50">
                      {col.links.map((link, lIdx) => (
                        <div key={lIdx} className="flex gap-2 items-center">
                          <input 
                            value={link.label} onChange={e => handleUpdateLink(cIdx, lIdx, 'label', e.target.value)}
                            className="text-xs text-slate-800 bg-white border border-green-100 p-2.5 rounded-xl w-1/3 font-bold outline-none focus:border-emerald-500 shadow-sm"
                            placeholder="Label" 
                          />
                          <input 
                            value={link.url} onChange={e => handleUpdateLink(cIdx, lIdx, 'url', e.target.value)}
                            className="text-xs text-emerald-600 font-mono bg-white border border-green-100 p-2.5 rounded-xl flex-1 outline-none focus:border-emerald-500 shadow-sm"
                            placeholder="/p/rute" 
                          />
                      ...
                      <button type="button" onClick={() => handleAddLinkToCol(cIdx)} className="text-[10px] font-black text-emerald-600 hover:text-emerald-700 hover:underline block pt-2 uppercase tracking-widest">
        ...
        <div className="bg-white p-6 lg:p-8 rounded-[2rem] rounded-tr-none border-2 border-green-50 shadow-sm space-y-6 hover:border-emerald-100 transition-colors">
          <div className="border-b-2 border-green-50 pb-4">
            <h2 className="text-sm font-black text-green-950 uppercase tracking-widest flex items-center gap-2">Komunikasi & Jejaring</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="text-[11px] font-black text-slate-500 mb-2 uppercase tracking-widest flex items-center gap-2"><Mail size={14} className="text-emerald-500"/> Email Operasional</label>
              <input value={settings.site_email || ''} onChange={e => handleChange('site_email', e.target.value)} className="w-full p-3.5 bg-white border border-green-100 rounded-2xl rounded-tr-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none text-sm font-mono font-bold text-slate-800 shadow-sm transition-all" placeholder="contact@desacitali.go.id" />
            </div>
            <div>
              <label className="text-[11px] font-black text-slate-500 mb-2 uppercase tracking-widest flex items-center gap-2"><Phone size={14} className="text-emerald-500"/> Kontak / WhatsApp</label>
            ...
            <div className="md:col-span-2">
              <label className="text-[11px] font-black text-slate-500 mb-2 uppercase tracking-widest flex items-center gap-2"><MapPin size={14} className="text-emerald-500"/> Alamat Kantor Pusat</label>
            ...
            <div>
              <label className="text-[11px] font-black text-slate-500 mb-2 uppercase tracking-widest flex items-center gap-2"><Share2 size={14} className="text-emerald-500"/> Facebook URL</label>
              <input value={settings.social_facebook || ''} onChange={e => handleChange('social_facebook', e.target.value)} className="w-full p-3.5 bg-white border border-green-100 rounded-2xl rounded-tr-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none text-sm font-mono font-medium text-emerald-600 shadow-sm transition-all" />
            </div>
            <div>
              <label className="text-[11px] font-black text-slate-500 mb-2 uppercase tracking-widest flex items-center gap-2"><Link2 size={14} className="text-emerald-500"/> Instagram URL</label>
            ...
            <div className="md:col-span-2">
              <label className="text-[11px] font-black text-slate-500 mb-2 uppercase tracking-widest flex items-center gap-2"><Share2 size={14} className="text-emerald-500"/> LinkedIn / Youtube URL</label>
        ...
        <button type="submit" disabled={isSaving} className="w-full py-4 bg-green-950 hover:bg-green-900 text-white font-black rounded-[2rem] rounded-tr-none text-sm uppercase tracking-widest shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all active:scale-95 flex items-center justify-center gap-3 group/btn">
          {isSaving ? <Loader2 className="animate-spin text-emerald-400" size={20} /> : <Save size={20} className="text-emerald-400 group-hover/btn:rotate-12 transition-transform" />} Terapkan Sinkronisasi Atribut
        </button>
      </form>
      ...
      {isSuccessModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-green-950/80 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[2rem] rounded-tr-none w-full max-w-sm shadow-2xl text-center p-8 border-4 border-white/20 animate-in zoom-in-95 duration-200">
            ...
            <button onClick={() => setIsSuccessModal(false)} className="mt-8 w-full py-3.5 bg-green-950 hover:bg-green-900 text-white font-black text-xs rounded-xl shadow-lg hover:shadow-xl transition-all uppercase tracking-widest active:scale-95">Tutup Panel</button>
          </div>
        </div>
      )}
    </div>
