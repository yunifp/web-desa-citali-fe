        ...
        <div className="space-y-8 font-sans animate-in fade-in duration-300 pb-10">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white p-8 rounded-[2rem] rounded-tr-none border-2 border-green-50 shadow-sm hover:border-emerald-100 transition-colors group">
                ...
                {canCreate && (
                    <button onClick={() => openModal()} className="bg-green-950 hover:bg-green-900 text-white px-6 py-3.5 rounded-2xl rounded-tr-none flex items-center gap-3 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all font-black text-xs uppercase tracking-widest active:scale-95 group/btn">
                        <Plus size={16} className="text-emerald-400 group-hover/btn:rotate-90 transition-transform" /> Tambah Template
                    </button>
                )}
            </div>
            ...
            <div className="bg-white rounded-[2rem] rounded-tr-none shadow-sm border-2 border-green-50 overflow-hidden flex flex-col hover:border-emerald-100 transition-colors">
                ...
                <div className="overflow-x-auto flex-1">
                    <table className="w-full text-left border-collapse min-w-[800px]">
                        ...
                        <tbody className="text-sm divide-y divide-green-50">
                            {isLoadingTemplates ? (
                                <tr><td colSpan={4} className="p-24 text-center text-slate-400 font-bold"><Loader2 className="animate-spin mx-auto mb-4 text-emerald-500" size={32} />Memuat spesifikasi...</td></tr>
                            ) : templates.length > 0 ? (
                                templates.map(item => (
                                    <tr key={item.id} className="hover:bg-green-50/50 transition-colors group">
                                        <td className="p-6 pl-8">
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 rounded-2xl rounded-tr-none bg-emerald-50 flex items-center justify-center flex-shrink-0 border border-emerald-100 shadow-sm group-hover:scale-105 transition-transform"><Monitor size={20} className="text-emerald-600" /></div>
                                                <span className="font-black text-slate-800 text-sm group-hover:text-green-950 transition-colors">{item.name}</span>
                                            </div>
                                        </td>
                                        <td className="p-6">
                                            <span className="flex items-center gap-2 bg-green-950 text-white font-mono text-[10px] font-black px-3 py-1.5 rounded-xl w-fit shadow-sm">
                                                <Code size={14} className="text-emerald-400" /> {item.slug}
                                            </span>
                                        </td>
                                ...
                                <tr><td colSpan={4} className="p-24 text-center text-slate-400 font-bold bg-slate-50 border-2 border-dashed border-green-100 rounded-[2rem]">Spesifikasi template kosong.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                ...
                <div className="flex items-center justify-between px-6 lg:px-8 py-5 bg-white border-t-2 border-green-50 rounded-b-[2rem] rounded-bl-none">
                    <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest border-l-2 border-emerald-400 pl-3">Total {templateMeta.totalItems} Entitas</span>
                    <div className="flex items-center gap-2 bg-slate-50 border border-green-100 rounded-2xl rounded-tr-none p-1.5 shadow-sm">
                        ...
                        <span className="px-4 py-2 text-[11px] font-black text-white bg-green-950 rounded-xl shadow-sm">{templateMeta.currentPage} / {templateMeta.totalPages || 1}</span>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-green-950/80 backdrop-blur-sm p-4 overflow-y-auto">
                    <div className="bg-white rounded-[2rem] rounded-tr-none w-full max-w-xl shadow-2xl flex flex-col border-4 border-white/20 animate-in zoom-in-95 duration-200">
                        <div className="px-8 py-6 border-b-2 border-green-50 flex justify-between items-center bg-white">
                            <div className="flex items-center gap-4">
                                <div className="p-3 bg-green-50 text-green-950 rounded-2xl rounded-tr-none border border-green-100 shadow-sm"><Monitor size={24} className="text-emerald-600" /></div>
                                <div>
                                    ...
                                    <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mt-1">Konfigurasi Modul</p>
                                </div>
                            </div>
                            ...
                            <form id="templateForm" onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <FileCode className="absolute left-5 top-1/2 -translate-y-1/2 text-emerald-600" size={18} />
                        ...
                        <div className="px-8 py-5 border-t-2 border-green-50 bg-white flex justify-end gap-4 rounded-b-[2rem]">
                            ...
                            <button form="templateForm" type="submit" className="px-8 py-3 bg-green-950 hover:bg-green-900 text-white font-black text-xs rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:scale-95 transition-all uppercase tracking-widest flex items-center gap-2">
                                Simpan Entitas
                            </button>
                        </div>
                    </div>
