        ...
        <div className="space-y-8 font-sans animate-in fade-in duration-300 pb-10">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white p-8 rounded-[2rem] rounded-tr-none border-2 border-green-50 shadow-sm hover:border-emerald-100 transition-colors group">
                ...
                {canCreate && (
                    ...
                    >
                        <Plus size={16} className="text-emerald-400 group-hover/btn:rotate-90 transition-transform" /> Buat Halaman Baru
                    </button>
                )}
            </div>

            {/* Tabel Penampang */}
            <div className="bg-white rounded-[2rem] rounded-tr-none border-2 border-green-50 overflow-hidden shadow-sm flex flex-col hover:border-emerald-100 transition-colors">
                    ...
                    <table className="w-full text-left border-collapse min-w-[800px]">
                        ...
                        <tbody className="text-sm divide-y divide-green-50">
                            {isLoading ? (
                                    ...
                                    <tr key={p.id} className="hover:bg-green-50/50 transition-colors group">
                                        <td className="p-6 max-w-xs">
                                            <div className="flex items-start gap-4">
                                                ...
                                                <div className="truncate py-1">
                                                    <div className="font-black text-slate-800 text-sm truncate group-hover:text-green-950 transition-colors" title={p.title}>{p.title}</div>
                                        ...
                                        <td className="p-6 whitespace-nowrap">
                                            <span className="bg-slate-50 text-slate-800 px-3 py-1.5 rounded-xl text-[11px] font-bold flex items-center gap-2 w-fit border border-slate-200 shadow-sm">
                                                <Layers size={14} className="text-emerald-600" /> {p.template?.name || 'Layout Default Statis'}
                                            </span>
                                        </td>
                                        <td className="p-6 whitespace-nowrap">
                                                ...
                                                <div className="flex justify-center gap-2">
                                                    {canUpdate && (
                                                        <button 
                                                            ...
                                                            className="p-2.5 text-green-900 hover:text-white hover:bg-green-950 rounded-xl transition-all shadow-sm hover:shadow-md" title="Edit Tata Letak"
                                                        >
                                                            <Edit size={16} />
                                                        </button>
                                                    )}
                                ...
                                <tr>
                                    <td colSpan={5} className="p-24 text-center text-slate-400 font-bold bg-slate-50 border-2 border-dashed border-green-100 rounded-[2rem]">
                ...
                {/* Footer Rekap */}
                <div className="px-6 lg:px-8 py-5 bg-white border-t-2 border-green-50 rounded-b-[2rem] rounded-bl-none text-[10px] text-slate-500 font-black uppercase tracking-widest">
                    <span className="border-l-2 border-emerald-400 pl-3">Total {pages.length} Halaman Statis Terdaftar</span>
                </div>
            </div>
