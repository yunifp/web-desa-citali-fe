    ...
    return (
        <div className="space-y-8 font-sans animate-in fade-in duration-300 pb-10">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white p-8 rounded-[2rem] rounded-tr-none border-2 border-green-50 shadow-sm hover:border-emerald-100 transition-colors group">
                ...
                >
                    <Plus size={16} className="text-emerald-400 group-hover/btn:rotate-90 transition-transform" /> Tambah Produk Baru
                </button>
            </div>
            ...
            <div className="bg-white rounded-[2rem] rounded-tr-none shadow-sm border-2 border-green-50 overflow-hidden flex flex-col hover:border-emerald-100 transition-colors">
                ...
                <div className="p-6 lg:p-8 border-b border-green-50 bg-slate-50/50 grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div className="overflow-x-auto flex-1">
                    <table className="w-full text-left border-collapse min-w-[800px]">
                        ...
                        <tbody className="text-sm divide-y divide-green-50">
                            {loading ? (
                                <tr><td colSpan={5} className="p-24 text-center text-slate-400 font-bold"><Loader2 className="animate-spin mx-auto mb-4 text-emerald-500" size={32} />Memuat katalog...</td></tr>
                            ) : products.length > 0 ? (
                                products.map(p => (
                                    <tr key={p.id} className="hover:bg-green-50/50 transition-colors group">
                                        <td className="p-6 max-w-xs">
                                            <div className="flex items-start gap-4">
                                                <div className="w-16 h-16 rounded-2xl rounded-tr-none border border-green-100 overflow-hidden bg-white flex-shrink-0 flex items-center justify-center shadow-sm">
                                                    {p.image ? (
                                                ...
                                                <div className="truncate py-1">
                                                    <div className="font-black text-slate-800 text-sm truncate group-hover:text-green-950 transition-colors" title={p.name}>{p.name}</div>
                                                    <div className="text-xs text-slate-500 truncate mt-1">{p.description || "Tidak ada deskripsi"}</div>
                                                    <div className="text-[10px] text-emerald-600 font-mono mt-2 px-2 py-1 bg-emerald-50 rounded-lg w-fit border border-emerald-100">{p.slug}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="p-6 whitespace-nowrap">
                                            <div className="flex items-center gap-2 font-black text-green-950 text-sm">
                                                <Tag size={16} className="text-emerald-500" /> Rp {p.price.toLocaleString("id-ID")}
                                            </div>
                                        </td>
                                        <td className="p-6 whitespace-nowrap text-center">
                                            <div className="inline-flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-xl border border-slate-200 font-black text-slate-700 shadow-sm">
                                                <Box size={16} className={p.stock > 0 ? "text-green-900" : "text-red-500"} />
                                                {p.stock}
                                            </div>
                                        </td>
                                        <td className="p-6 whitespace-nowrap text-center">
                                        ...
                                        <td className="p-6 whitespace-nowrap">
                                            <div className="flex justify-center gap-2">
                                                <button
                                                    ...
                                                    className="p-2.5 text-green-900 hover:text-white hover:bg-green-950 rounded-xl transition-all shadow-sm hover:shadow-md" title="Edit Produk"
                                                >
                                                    <Edit size={16} />
                                                </button>
                                                <button
                            ...
                            ) : (<tr><td colSpan={5} className="p-24 text-center text-slate-400 font-bold bg-slate-50 border-2 border-dashed border-green-100 rounded-[2rem]">Produk belum tersedia di etalase.</td></tr>)}
                        </tbody>
                    </table>
                </div>
                ...
                <div className="flex items-center justify-between px-6 lg:px-8 py-5 bg-white border-t-2 border-green-50 rounded-b-[2rem] rounded-bl-none">
                    <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest border-l-2 border-emerald-400 pl-3">Total {meta.totalItems || products.length} Produk</span>
            ...
            ========================================================= */}
            {showFormModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-green-950/80 backdrop-blur-sm p-4 sm:p-6 overflow-y-auto">
                    <div className="bg-white rounded-[2rem] rounded-tr-none w-full max-w-4xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden border-4 border-white/20 animate-in zoom-in-95 duration-200">
                        ...
                        <div className="px-8 py-6 border-b-2 border-green-50 flex justify-between items-center bg-white">
                            <div className="flex items-center gap-4">
                                <div className="p-3 bg-green-50 text-green-950 rounded-2xl rounded-tr-none border border-green-100 shadow-sm"><ShoppingBag size={24} className="text-emerald-600" /></div>
                                <div>
                                    <h2 className="text-xl font-light text-slate-800 leading-tight">
                                        {editingProduct ? "Edit Informasi Produk" : "Tambah Produk UMKM Baru"}
                                    </h2>
                                    <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mt-1">Katalog Etalase Desa</p>
                                </div>
                            </div>
                        ...
                        <div className="p-8 overflow-y-auto bg-slate-50/50 flex-1">
                            <form id="productForm" onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
                                ...
                                <div className="md:col-span-2 space-y-6">
                                    ...
                                    <div className="grid grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <div className="relative">
                                                <span className="absolute left-5 top-1/2 -translate-y-1/2 text-emerald-600 font-black text-sm">Rp</span>
                                                <input type="number" required value={formData.price} onChange={e => setFormData(p => ({ ...p, price: Number(e.target.value) }))} className="w-full pl-12 pr-5 py-3.5 bg-white border border-green-100 rounded-2xl rounded-tr-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none text-sm font-black text-slate-900 shadow-sm transition-all" />
                                            </div>
                                        </div>
                                        <div>
                                            <label className="block text-[11px] font-black text-slate-500 mb-2 uppercase tracking-widest">Stok Barang *</label>
                                            <div className="relative">
                                                <Box className="absolute left-5 top-1/2 -translate-y-1/2 text-emerald-600" size={16} />
                                    ...
                                    <h3 className="text-sm font-black text-green-950 border-b-2 border-green-50 pb-3 pt-6 flex items-center gap-2">
                                        <Link2 size={16} className="text-emerald-500" /> Konfigurasi Tombol Checkout
                                    </h3>
                                    
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-white p-6 rounded-[2rem] rounded-tr-none border border-green-50 shadow-sm">
                                        <div>
                                            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Label Tombol 1</label>
                                            <input type="text" value={formData.button1Label} onChange={e => setFormData(p => ({ ...p, button1Label: e.target.value }))} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-emerald-500 outline-none text-xs font-bold transition-colors" />
                                            ...
                                            <div className="relative">
                                                ...
                                                <input type="url" value={formData.button1Url} onChange={e => setFormData(p => ({ ...p, button1Url: e.target.value }))} className="w-full pl-10 p-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-emerald-500 outline-none text-xs transition-colors" placeholder="https://wa.me/628..." />
                                            </div>
                                        </div>
                                        <div>
                                            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Label Tombol 2</label>
                                            <input type="text" value={formData.button2Label} onChange={e => setFormData(p => ({ ...p, button2Label: e.target.value }))} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-emerald-500 outline-none text-xs font-bold transition-colors" />
                                            ...
                                            <div className="relative">
                                                ...
                                                <input type="url" value={formData.button2Url} onChange={e => setFormData(p => ({ ...p, button2Url: e.target.value }))} className="w-full pl-10 p-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-emerald-500 outline-none text-xs transition-colors" placeholder="https://shopee.co.id/..." />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-6">
                                    <h3 className="text-sm font-black text-green-950 border-b-2 border-green-50 pb-3 inline-flex items-center gap-2">
                                        <ImageIcon size={16} className="text-emerald-500" /> Media & Etalase
                                    </h3>

                                    <div className="bg-white p-5 rounded-2xl rounded-tr-none border border-green-50 shadow-sm">
                                        <label className="block text-[11px] font-black text-slate-500 mb-2 uppercase tracking-widest">Status Tayang</label>
                                        <select value={formData.status} onChange={e => setFormData(p => ({ ...p, status: e.target.value as any }))} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none text-sm font-black text-slate-800 cursor-pointer transition-all">
                                    ...
                                    <div className="bg-white p-5 rounded-[2rem] rounded-tr-none border border-green-50 shadow-sm">
                                        ...
                                        <div className="grid grid-cols-3 gap-3 mb-4">
                                            {formData.images.map((imgUrl, idx) => (
                                                <div key={idx} className={`relative aspect-square rounded-xl border-2 overflow-hidden group transition-all ${formData.image === imgUrl ? 'border-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.3)]' : 'border-slate-200'}`}>
                                                    <img src={getImageUrl(imgUrl)} alt="Preview" className="w-full h-full object-cover" />
                                                    <div className="absolute inset-0 bg-green-950/60 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity gap-2 backdrop-blur-sm">
                                                        {formData.image !== imgUrl && (
                                                            <button type="button" onClick={() => setAsMainImage(imgUrl)} className="text-[9px] font-black uppercase text-white bg-emerald-500 hover:bg-emerald-400 px-3 py-1.5 rounded-lg shadow-sm transition-colors">Jadikan Utama</button>
                                                        )}
                                                    ...
                                                    {formData.image === imgUrl && (
                                                        <div className="absolute top-1.5 left-1.5 bg-emerald-500 text-white text-[8px] font-black px-2 py-0.5 rounded shadow-sm">UTAMA</div>
                                                    )}
                                                </div>
                                            ))}
                                            
                                            {formData.images.length < 5 && (
                                                <div className="border-2 border-dashed border-green-200 rounded-xl bg-green-50/50 hover:bg-green-50 hover:border-emerald-400 transition-colors flex flex-col items-center justify-center relative cursor-pointer aspect-square group">
                                                    ...
                                                    {uploading ? (
                                                        ...
                                                        <>
                                                            <Plus size={24} className="text-emerald-500 mb-1 group-hover:scale-110 transition-transform" />
                                                            <span className="text-[10px] font-black text-green-900">Tambah</span>
                                                        </>
                                                    )}
                                                </div>
                                            )}
                                        </div>
                                        <p className="text-[10px] text-slate-500 leading-relaxed font-medium bg-green-50 p-3 rounded-xl border border-green-100">
                                            Gambar pertama otomatis menjadi cover utama. Anda bisa mengubah cover utama dengan hover dan klik <strong className="text-emerald-600">"Jadikan Utama"</strong>.
                                        </p>
                                    </div>
                                </div>
                        ...
                        <div className="px-8 py-5 border-t-2 border-green-50 bg-white flex justify-end gap-4 rounded-b-[2rem]">
                            ...
                            <button form="productForm" type="submit" className="px-8 py-3 bg-green-950 hover:bg-green-900 text-white font-black text-xs rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:scale-95 transition-all uppercase tracking-widest flex items-center gap-2">
                                {editingProduct ? 'Simpan Perubahan' : 'Rilis Produk'} <ArrowRight size={16} className="text-emerald-400" />
                            </button>
                        </div>
                    </div>
                </div>
