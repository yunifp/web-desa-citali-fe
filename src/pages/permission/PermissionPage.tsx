        ...
        <div className="space-y-8 font-sans animate-in fade-in duration-300 pb-10">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white p-8 rounded-[2rem] rounded-tr-none border-2 border-green-50 shadow-sm hover:border-emerald-100 transition-colors group">
                ...
                {canCreate && (
                    ...
                    >
                        <Plus size={16} className="text-emerald-400 group-hover/btn:rotate-90 transition-transform" /> Tambah Izin Aksi
                    </button>
                )}
            </div>
            ...
            <div className="bg-white rounded-[2rem] rounded-tr-none shadow-sm border-2 border-green-50 overflow-hidden flex flex-col hover:border-emerald-100 transition-colors">
                ...
                <div className="overflow-x-auto flex-1">
                    <table className="w-full text-left border-collapse min-w-[600px]">
                        <thead className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b-2 border-green-50">
                            <tr>
                                <th
                                    className="p-6 pl-8 cursor-pointer hover:text-emerald-600 transition-colors w-1/2 whitespace-nowrap"
                        ...
                        <tbody className="divide-y divide-green-50 text-sm">
                            {isLoading ? (
                                <tr><td colSpan={canDelete ? 3 : 2} className="p-24 text-center text-slate-400 font-bold"><Loader2 className="animate-spin mx-auto mb-4 text-emerald-500" size={32} />Memuat registri...</td></tr>
                            ) : paginatedData.length > 0 ? (
                                paginatedData.map((p) => (
                                    <tr key={p.id} className="hover:bg-green-50/50 transition-colors group">
                                        <td className="p-6 pl-8">
                                            <div className="flex items-center gap-4">
                                                <div className="w-10 h-10 bg-green-950 text-emerald-400 rounded-xl flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform">
                                                    <Key size={18} />
                                                </div>
                                                <span className="font-black text-slate-800 group-hover:text-green-950 transition-colors uppercase tracking-widest">{p.name}</span>
                                            </div>
                                        </td>
                                        <td className="p-6 text-[11px] font-mono font-bold text-emerald-600 bg-emerald-50/50 rounded-lg px-3 py-1 w-fit border border-emerald-100">{p.id}</td>
                                        {canDelete && (
                                ...
                                <tr><td colSpan={canDelete ? 3 : 2} className="p-24 text-center text-slate-400 font-bold bg-slate-50 border-2 border-dashed border-green-100 rounded-[2rem]">Registri tindakan kosong.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                ...
                {totalPages > 0 && (
                    <div className="flex items-center justify-between px-6 lg:px-8 py-5 bg-white border-t-2 border-green-50 rounded-b-[2rem] rounded-bl-none">
                        <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest border-l-2 border-emerald-400 pl-3">Total {processedData.length} Izin</span>
                        <div className="flex items-center gap-2 bg-slate-50 border border-green-100 rounded-2xl rounded-tr-none p-1.5 shadow-sm">
                            ...
                            <span className="px-4 py-2 text-[11px] font-black text-white bg-green-950 rounded-xl shadow-sm">{currentPage} / {totalPages}</span>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-green-950/80 backdrop-blur-sm p-4 overflow-y-auto">
                    <div className="bg-white rounded-[2rem] rounded-tr-none w-full max-w-sm shadow-2xl overflow-hidden border-4 border-white/20 animate-in zoom-in-95 duration-200">
                        <div className="p-6 lg:p-8 bg-green-950 text-white flex justify-between items-center border-b-2 border-green-900">
                            <h2 className="text-sm font-black tracking-widest uppercase">Injeksi Parameter Izin</h2>
                            <button onClick={() => setIsModalOpen(false)} className="text-emerald-400 hover:text-white p-2 bg-green-900 rounded-xl transition-colors"><X size={16} /></button>
                        </div>
                        <form onSubmit={handleSubmit} className="p-6 lg:p-8 space-y-6">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full p-3.5 bg-slate-50 border border-green-100 rounded-2xl rounded-tr-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none text-sm font-black uppercase text-slate-800 shadow-sm transition-all placeholder:normal-case placeholder:font-normal"
                                    placeholder="e.g. EXPORT, VALIDATE"
                                />
                                <p className="text-[10px] text-emerald-600 mt-2 font-black uppercase tracking-widest bg-emerald-50 p-2 rounded-lg border border-emerald-100">* Format standarisasi global adalah UPPERCASE.</p>
                            </div>
                            <button type="submit" className="w-full py-3.5 bg-emerald-500 hover:bg-emerald-400 text-white font-black rounded-2xl rounded-tr-none text-xs uppercase tracking-widest shadow-md transition-all active:scale-95">
                                Simpan Parameter
                            </button>
                        </form>
                    </div>
