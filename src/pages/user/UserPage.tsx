        ...
        <div className="space-y-8 font-sans animate-in fade-in duration-300 pb-10">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white p-8 rounded-[2rem] rounded-tr-none border-2 border-green-50 shadow-sm hover:border-emerald-100 transition-colors group">
                ...
                {canCreate && (
                    <button onClick={() => openModal()} className="bg-green-950 hover:bg-green-900 text-white px-6 py-3.5 rounded-2xl rounded-tr-none flex items-center gap-3 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all font-black text-xs uppercase tracking-widest active:scale-95 group/btn">
                        <Plus size={16} className="text-emerald-400 group-hover/btn:rotate-90 transition-transform" /> Registrasi Staf
                    </button>
                )}
            </div>
            ...
            <div className="bg-white rounded-[2rem] rounded-tr-none shadow-sm border-2 border-green-50 overflow-hidden flex flex-col hover:border-emerald-100 transition-colors">
                ...
                <div className="overflow-x-auto flex-1">
                    <table className="w-full text-left border-collapse min-w-[800px]">
                        ...
                        <tbody className="text-sm divide-y divide-green-50">
                            {isLoading ? (
                                <tr><td colSpan={4} className="p-24 text-center text-slate-400 font-bold"><Loader2 className="animate-spin mx-auto mb-4 text-emerald-500" size={32} />Memuat registri staf...</td></tr>
                            ) : processedUsers.length > 0 ? (
                                processedUsers.map(userItem => (
                                    <tr key={userItem.id} className="hover:bg-green-50/50 transition-colors group">
                                        <td className="p-6 pl-8 font-black text-slate-800 group-hover:text-green-950 transition-colors">{userItem.name}</td>
                                        <td className="p-6 text-slate-500 font-medium">{userItem.email}</td>
                                        <td className="p-6">
                                            <div className="flex flex-wrap gap-2">
                                                {userItem.roles.map((ur: any, idx: number) => (
                                                    <span key={idx} className="bg-green-950 text-white font-black text-[10px] px-3 py-1.5 rounded-xl shadow-sm uppercase tracking-widest">
                                        ...
                                        {(canUpdate || canDelete) && (
                                            <td className="p-6 pr-8">
                                                <div className="flex justify-center gap-2">
                                                    {canUpdate && <button onClick={() => openModal(userItem)} className="p-2.5 text-green-900 hover:text-white hover:bg-green-950 rounded-xl transition-all shadow-sm hover:shadow-md" title="Koreksi"><Edit size={16} /></button>}
                                ...
                                <tr><td colSpan={4} className="p-24 text-center text-slate-400 font-bold bg-slate-50 border-2 border-dashed border-green-100 rounded-[2rem]"><Inbox className="mx-auto mb-4 text-slate-300" size={48} /> <p>Arsip staf pengguna kosong.</p></td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                ...
                <div className="flex items-center justify-between px-6 lg:px-8 py-5 bg-white border-t-2 border-green-50 rounded-b-[2rem] rounded-bl-none">
                    <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest border-l-2 border-emerald-400 pl-3">Total {meta.totalItems} Pengguna</span>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-green-950/80 backdrop-blur-sm p-4 overflow-y-auto">
                    <div className="bg-white rounded-[2rem] rounded-tr-none w-full max-w-xl shadow-2xl flex flex-col max-h-[90vh] border-4 border-white/20 animate-in zoom-in-95 duration-200">
                        <div className="px-8 py-6 border-b-2 border-green-50 flex justify-between items-center bg-white">
                            <div className="flex items-center gap-4">
                                <div className="p-3 bg-green-50 text-green-950 rounded-2xl rounded-tr-none border border-green-100 shadow-sm"><Shield size={24} className="text-emerald-600" /></div>
                                <div>
                                    ...
                                    <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mt-1">Sistem Keamanan</p>
                                </div>
                            </div>
                        ...
                        <div className="p-8 bg-slate-50/50 overflow-y-auto custom-scrollbar flex-1">
                            <form id="userForm" onSubmit={handleSubmit} className="space-y-6">
                                ...
                                {!currentUser && (
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-emerald-600 transition-colors" tabIndex={-1}>
                                ...
                                <div className="space-y-3 pt-4 border-t-2 border-green-50">
                                    <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2"><Shield size={14} className="text-emerald-600" /> Pengikatan Kelompok Wewenang</label>
                                    <div className="grid grid-cols-2 gap-3 bg-white p-4 rounded-[2rem] rounded-tr-none border border-green-50 shadow-sm">
                                        {roles.map(role => (
                                            <label key={role.id} className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all border-2 ${formData.roleIds.includes(role.id) ? 'bg-emerald-50 border-emerald-400 text-green-950 shadow-sm' : 'bg-slate-50 border-green-50 text-slate-600 hover:border-emerald-200'}`}>
                        ...
                        <div className="px-8 py-5 border-t-2 border-green-50 bg-white flex justify-end gap-4 rounded-b-[2rem]">
                            ...
                            <button form="userForm" type="submit" className="px-8 py-3 bg-green-950 hover:bg-green-900 text-white font-black text-xs rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:scale-95 transition-all uppercase tracking-widest">
            ...
            {isErrorModalOpen && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-green-950/80 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-[2rem] rounded-tr-none w-full max-w-sm shadow-2xl text-center p-8 border-4 border-white/20 animate-in zoom-in-95 duration-200">
                        ...
                        <button onClick={() => setIsErrorModalOpen(false)} className="w-full py-3.5 bg-green-950 hover:bg-green-900 text-white font-black text-xs rounded-xl shadow-lg hover:shadow-xl transition-all uppercase tracking-widest active:scale-95">Tutup Jendela</button>
                    </div>
                </div>
            )}
